...
fn test_unlock_liquidity() {
    ...
    // test params 
    let inital_unborrowed_cap = 1000u64;
    let after_unborrowed_cap = 500u64;
    let ckb_tot_sup = 1000u128;
    let cckb_tot_sup = 500u128;
    let ckb_tot_sup_a = 500u128;
    let cckb_tot_sup_a = 250u128;
    let cckb_in_amount = 250u128;
    let unlock_capacity = 500u64;
    ...
    // prepare sudt deposit cell
    let input_cckb_capacity = cckb_in_amount.to_le_bytes().to_vec().len() as u64;
    let input_out_point_1 = context.create_cell(
        CellOutput::new_builder()
            .capacity(input_cckb_capacity.pack())
            .lock(lock_script_as.clone())
            .type_(Some(lock_script_sudt.clone()).pack())
            .build(),
        cckb_in_amount.to_le_bytes().to_vec().into(),
    );
    let input1 = CellInput::new_builder()
        .previous_output(input_out_point_1)
        .build();
    ...
    let outputs = vec![
        CellOutput::new_builder()
            ...
            .build(),
        CellOutput::new_builder()
            .capacity(unlock_capacity.pack())
            .lock(lock_script_as.clone())
            .build(),
    ];

    let outputs_data : Vec<Bytes> = vec![ckb_total_supply_a.into(), Bytes::new()];
